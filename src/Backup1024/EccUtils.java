...
public class EccUtils {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("解密后clearTxt:" + new String(clearTxt));

            //签名
            byte[] sign = sign(content, priKey).getBytes();
            //验签
    ...
    public static String encrypt(String content, ECPublicKey pubKey)throws Exception {
        Cipher cipher = Cipher.getInstance("ECIES", "BC");
        cipher.init(Cipher.ENCRYPT_MODE, pubKey);
        byte[] ensign=content.getBytes();
        return cipher.doFinal(ensign).toString();
    }
    ...
    public static String decrypt(String content, ECPublicKey pubKey)throws Exception {
        Cipher cipher = Cipher.getInstance("ECIES", "BC");
    ...
    public static String sign(String content, ECPrivateKey priKey) throws Exception {
        //这里可以从证书中解析出签名算法名称
        //Signature signature = Signature.getInstance(getSigAlgName(pubCert));
        Signature signature = Signature.getInstance(SIGNATURE);//"SHA256withECDSA"
        signature.initSign(priKey);
